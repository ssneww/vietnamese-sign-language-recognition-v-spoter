...
def __preprocess_row_sign(sign: dict) -> Tuple[dict, dict]:
    """
...
def augment_rotate_hands_around_wrist(sign: dict, angle: float) -> dict:
    """
    Xoay bàn tay quanh góc cụ thể
    """

    # Tách dữ liệu xương khớp thành body_landmarks và hand_landmarks
    body_landmarks, hand_landmarks = __preprocess_row_sign(sign)
    angle_radians = math.radians(angle)  # Chuyển đổi góc từ độ sang radian

    # Xoay các khớp bàn tay quanh khớp cổ tay cho cả hai tay (trái và phải)
    for side in ["left", "right"]:
        wrist_identifier = f"{side}Wrist"

        if wrist_identifier in body_landmarks:
            wrist_landmarks = body_landmarks[wrist_identifier]

            # Xoay các khớp bàn tay quanh cổ tay
            for hand_key, hand_frames in hand_landmarks.items():
                if hand_key.startswith(side):  # Chỉ xử lý các khớp bàn tay của bên tương ứng (trái hoặc phải)
                    hand_landmarks[hand_key] = [__rotate(wrist_landmarks[frame_index], frame, angle_radians)
                                                for frame_index, frame in enumerate(hand_frames)]

    # Kết hợp lại dữ liệu khớp cơ thể và khớp bàn tay (chỉ xoay bàn tay quanh cổ tay)
    return __wrap_sign_into_row(body_landmarks, hand_landmarks)
